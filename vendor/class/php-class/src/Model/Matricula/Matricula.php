<?php 
    ...
    class Matricula extends Model{
        ...
        public static function listAllMatricula($idmatricula){
            $sql = new Sql();
            $result = $sql->select("SELECT nomeuser, emailuser, imguser, telfixouser, celuser, sexouser, estadocivil, naturalidade, nomepai, nomemae, documento, cpf, escolaridade, profissao, igreja, pastor, funcao, conversao, status, nomecurso, statuscurso, duracao, nometurma, statusturma, vencimentoboleto, datamatricula, statusmatricula FROM matricula INNER JOIN user USING(iduser) INNER JOIN curso USING (idcurso) INNER JOIN turma USING(idturma) WHERE idmatricula =:id", array(
                ":id" => $idmatricula
            ));
